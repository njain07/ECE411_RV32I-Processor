...
_start:
    ...
    add  x3, x4, x6
    la   x10, SVAL1
    sw   x3, 0(x10) #SVAL1 <- 33
    addi x6, x0, -10
    add  x4, x4, x6
    la   x10, SVAL2
    sw   x4, 0(x10) #SVAL2 <- 25
    addi x6, x0, -13
    and  x5, x2, x6
    la   x10, SVAL3
    sw   x5, 0(x10) #SVAL3 <- d1
    addi x6, x0, 12
    and  x6, x2, x6
    or   x6, x6, x0
    la   x10, SVAL4
    ...
    xor  x11, x8, x9
    la   x12, SVAL5
    sw   x11, 0(x12) #SVAL5 <- ff0
    sll  x11, x9, x10
    la   x12, SVAL6
    sw   x11, 0(x12) #SVAL6 <- ff0
    srl  x11, x9, x10
    la   x12, SVAL7
    sw   x11, 0(x12) #SVAL7 <- f
